...
Before outlining, work through `playbooks/insight-finding.md` against the
digests: deltas, outliers, streaks, mix shifts, "what would surprise this
audience," the buried lede. Write 3-5 candidate insights into the brief with
their supporting numbers. If the data is thin, say so rather than inventing.
...
- Set `animate: build` only where sequencing helps the story
  (KPI reveals, option comparisons); `fade` for gentle polish; default none.
