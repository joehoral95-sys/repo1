...
How to structure what the deck says. Sources distilled: Minto's pyramid
principle, assertion-evidence design, BLUF military briefing doctrine,
Duarte's resonate structure.
...
- Detail supports the answer; it never precedes it. Appendix > body for
  anything the audience *might* ask about.
...
Rules of thumb:
- If nobody could disagree with the title, it isn't an assertion.
- One assertion per slide. Two messages = two slides.
- Numbers make assertions stronger ("dipped 7%" beats "dipped").

## Pyramid structure

State the conclusion, then group supporting points (3 ± 1), each of which is
itself a mini-conclusion over its evidence.
- Deck level: conclusion slide(s) → grouped support sections (`section`
  dividers) → evidence slides.
- Slide level: assertion title → 3-ish supporting bullets/visual.
- Ordering within groups: by importance, not chronology, unless the story is
  genuinely temporal (then use `timeline`).
...
- Slides carry messages; speaker notes carry prose. ~40 words max per slide.
- Concrete beats abstract: "saves $0.9M" not "significant savings."
